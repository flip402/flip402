'use client';

import { useEffect, useState } from 'react';
import Image from 'next/image';
import { ConnectButton } from '@/components/wallet/ConnectButton';

export function Header() {
  const [scrolled, setScrolled] = useState(false);

  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 12);
    onScroll();
    window.addEventListener('scroll', onScroll, { passive: true });
    return () => window.removeEventListener('scroll', onScroll);
  }, []);

  return (
    <header
      className={`fixed left-0 right-0 top-0 z-40 transition-all duration-300 ${
        scrolled
          ? 'border-b border-border-bright bg-void/35 backdrop-blur-2xl backdrop-saturate-150'
          : 'border-b border-transparent bg-transparent'
      }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-3.5">
        <a href="#top" className="group flex items-center gap-2.5 sm:gap-3">
          <LogoMark />
          <span className="font-display text-xl font-bold tracking-tight sm:text-[22px]">
            FLIP<span className="grad-ice">402</span>
          </span>
        </a>

        <nav className="hidden items-center gap-5 text-base text-text-mute md:flex">
          <a
            href="https://twitter.com/flip402"
            target="_blank"
            rel="noreferrer noopener"
            aria-label="Twitter"
            className="text-text-mute transition-colors duration-200 hover:text-text"
          >
            <XIcon />
          </a>
          <a
            href="https://github.com/x402-foundation/x402"
            target="_blank"
            rel="noreferrer noopener"
            aria-label="GitHub"
            className="text-text-mute transition-colors duration-200 hover:text-text"
          >
            <GithubIcon />
          </a>
          <a
            href="https://x402.org"
            target="_blank"
            rel="noreferrer noopener"
            className="text-text-mute transition-colors duration-200 hover:text-text"
          >
            x402
          </a>
        </nav>

        <div className="flex shrink-0 items-center gap-2">
          <ConnectButton />
        </div>
      </div>
    </header>
  );
}

function XIcon() {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
    >
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

function GithubIcon() {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
    >
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

function LogoMark() {
  return (
    <span className="relative flex h-9 w-9 overflow-hidden rounded-lg sm:h-11 sm:w-11 sm:rounded-xl">
      <Image
        src="/flip402-mark.png"
        alt="FLIP402"
        width={88}
        height={88}
        priority
        className="h-full w-full object-cover"
      />
    </span>
  );
}
